...
"""Loads the image for foreground detection"""
filename = os.path.join("C:\Users\Admin\Desktop\Foreground", "Pear.jpg")
        ...
        for col in range(width):
            ...
            for neighbor in forward_pixel_neighbors([row, col], height, width):
                ...
                if x_val < 20:
                    y_lists[2][x_val] += 1
    new_x_list = x_list[:]
    for index in range(len(new_x_list)):
        new_x_list[index] = new_x_list[index]/100.0
...
def layered_foreground(image_list, min_prob_2):
    ...
    for index in range(len(image_list)):
        ...
        min_prob_1 = min_prob(current_image)
